...
function parseOfficeHours(raw: string | null): OfficeHourDraft[] {
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? (parsed as OfficeHourDraft[]) : [];
  } catch {
    return [];
  }
}

export default function TeacherEditScreen() {
  ...
  useEffect(() => {
    ...
    setEmail(t.email ?? '');
    setOfficeLocation(t.office_location ?? '');
    setTeachingStyle(t.teaching_style_notes ?? '');
    setGradingNotes(t.grading_notes ?? '');
    setRecPotential(t.rec_potential ?? 0);
    setRating(t.rating ?? 0);
    setNotes(t.notes_md ?? '');
    setOfficeHours(parseOfficeHours(t.office_hours));
    setLoaded(true);
  }, [db, id]);
  ...
  const handleSave = useCallback(() => {
    ...
    setSaving(true);
    try {
      updateTeacher(db, id, parsed.data);
      router.back();
    } catch (err) {
  ...
  if (missing) {
    return (
      <View style={styles.errorScreen}>
        <Text variant="heading">Teacher not found</Text>
        <Pressable
