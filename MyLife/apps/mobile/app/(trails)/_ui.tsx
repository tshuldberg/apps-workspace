...
export const TRAILS_ACCENT = colors.modules.trails;
  ...
  return (
    <View style={styles.screen}>
      <View pointerEvents="none" style={styles.backdrop}>
        <LinearGradient
          colors={['rgba(101,163,13,0.3)', 'rgba(101,163,13,0.08)', 'rgba(10,10,15,0)']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
...
export function TrailsGlassCard({
  children,
  style,
...
export function TrailsChip({
  ...
  return (
    <Pressable onPress={onPress} style={[styles.chip, active && styles.chipActive]}>
      <Text variant="caption" style={{ color: active ? '#112006' : colors.textSecondary, fontWeight: '600' }}>
        {label}
      </Text>
    </Pressable>
  );
...
export function TrailsPrimaryButton({
  label,
  onPress,
  style,
}: {
...
export function TrailsEmptyState({
  ...
  return (
    <TrailsGlassCard style={styles.emptyCard}>
...
const styles = StyleSheet.create({
  ...
  hero: {
    ...
    backgroundColor: 'rgba(18,25,10,0.86)',
    gap: spacing.xs,
  },
  heroHeader: {
    flexDirection: 'row',
  ...
  chipActive: {
    borderColor: 'rgba(101,163,13,0.3)',
    backgroundColor: 'rgba(101,163,13,0.88)',
  },
  primaryButton: {
    alignSelf: 'flex-start',
    borderRadius: 999,
    backgroundColor: TRAILS_ACCENT,
