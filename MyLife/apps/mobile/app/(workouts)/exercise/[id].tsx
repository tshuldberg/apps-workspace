...
export default function WorkoutExerciseDetailScreen() {
  ...
  const [selectedVideo, setSelectedVideo] = useState<DetailVideoItem | null>(null);
  ...
  const state = useMemo<DetailState>(() => {
    try {
      ...
      const exerciseVideos = getExerciseVideos(db, params.id);
      const fallbackVideo = exercise?.videoUrl
        ? [{
            id: `${exercise.id}-coach`,
            title: 'Coach demo',
      ...
      const videos = [
        ...exerciseVideos.map((video: ExerciseVideo) => ({
          id: video.id,
          title: video.angle.replace(/_/g, ' '),
          url: video.videoUri,
      ...
      if (exercise) {
        pushWorkoutRecentView(db, {
          ...
          route: `/(workouts)/exercise/${exercise.id}`,
  ...
  const handleShare = useCallback(async () => {
    if (!exercise) return;
    const deepLink = `mylife://workouts/exercise/${exercise.id}`;
    ...
    <View style={styles.screen}>
        ...
        <View style={styles.body}>
          <View style={styles.heroWrap}>
            <ExerciseArtwork
              title={exercise.name}
              accent={accent}
              uri={state.videos[0]?.thumbnailUri ?? exercise.thumbnailUrl}
              height={260}
            />
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <SectionLabel accent={WK_ACCENT_LIGHT}>Video Carousel</SectionLabel>
              <RNText style={styles.sectionMeta}>{state.videos.length} angles</RNText>
            </View>
                ...
                {state.videos.map((video) => (
                  ...
                  >
                    ...
                    <View style={styles.videoCopy}>
                      <RNText style={styles.videoTitle}>{video.title}</RNText>
                      <RNText style={styles.videoBody}>Tap to open demo</RNText>
                    </View>
                  </GlassPanel>
                ))}
              </ScrollView>
            ) : (
              <GlassPanel style={styles.emptyBlock}>
                <RNText style={styles.emptyBlockTitle}>No demo video yet</RNText>
                <RNText style={styles.emptyBlockBody}>Upload or attach a demo to make coaching cues visual.</RNText>
              </GlassPanel>
            )}
          </View>
      ...
      >
        <View style={styles.modalBackdrop}>
          <Pressable style={StyleSheet.absoluteFillObject} onPress={() => setSelectedVideo(null)} />
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalCopy}>
              <RNText style={styles.modalTitle}>{selectedVideo?.title ?? 'Demo video'}</RNText>
              <RNText style={styles.modalBody}>Open the reference clip to review setup and tempo.</RNText>
            </View>
            <View style={styles.modalActions}>
              <Pressable onPress={() => setSelectedVideo(null)} style={styles.modalSecondary}>
                <RNText style={styles.modalSecondaryText}>Close</RNText>
              </Pressable>
              <Pressable
                onPress={() => {
                  if (selectedVideo?.url) {
                    void Linking.openURL(selectedVideo.url);
                  }
                }}
                style={styles.modalPrimary}
              >
                <RNText style={styles.modalPrimaryText}>Open Demo</RNText>
              </Pressable>
            </View>
          </View>
        </View>
          ...
          <View style={styles.sheetCard}>
            ...
            <Pressable
              onPress={() => {
                ...
                router.push(`/(workouts)/builder?exerciseId=${exercise.id}` as never);
              }}
            ...
            {state.workouts.slice(0, 3).map((workout) => (
              <Pressable
                ...
                onPress={() => {
                  setShowWorkoutPicker(false);
                  router.push(`/(workouts)/builder?edit=${workout.id}&exerciseId=${exercise.id}` as never);
                }}
                style={styles.sheetRow}
              >
                <View style={{ flex: 1 }}>
