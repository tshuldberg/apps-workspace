...
export default function ProgramDetailScreen() {
  ...
  const handleShare = async () => {
    if (!plan) return;
    await Clipboard.setStringAsync(`mylife://workouts/program/${plan.id}`);
  ...
  const handlePrimaryAction = () => {
    ...
    if (todaysSchedule?.workout_id) {
      router.push(`/(workouts)/session?workoutId=${todaysSchedule.workout_id}` as never);
      return;
    }

    router.push('/(workouts)/recovery' as never);
  };
        ...
        <View style={styles.body}>
          <View style={styles.heroWrap}>
            ...
            <View style={styles.heroOverlay}>
              <SectionLabel accent={WK_ACCENT_LIGHT}>Advanced Strength</SectionLabel>
          ...
          {isSubscribed && progress && position ? (
            <GlassPanel style={styles.progressCard}>
              <View style={styles.progressHeader}>
                <View>
                  <SectionLabel accent={WK_ACCENT_LIGHT}>Active Subscription</SectionLabel>
          ...
          <GlassPanel style={styles.overviewCard}>
            <View style={styles.overviewGrid}>
              <View style={styles.metricBlock}>
                <MaterialSymbol name="schedule" size={18} color={WK_ACCENT_LIGHT} />
                <RNText style={styles.metricLabel}>Duration</RNText>
                <RNText style={styles.metricValue}>{plan.weeks.length} weeks</RNText>
              </View>
              <View style={styles.metricBlock}>
                <MaterialSymbol name="calendar_today" size={18} color={WK_ACCENT_LIGHT} />
          ...
          <View style={styles.sectionHeader}>
            <SectionLabel accent={WK_ACCENT_LIGHT}>Week Schedule</SectionLabel>
                    ...
                    {week.days.map((day, index) => {
                      ...
                      return (
                        <Pressable
                          ...
                          onPress={() => {
                            if (day.workout_id) {
                              router.push(`/(workouts)/session?workoutId=${day.workout_id}` as never);
                            }
                          }}
                          style={[
                            styles.dayRow,
                        ...
                        >
                          <View style={[
                            ...
                            <MaterialSymbol
                              ...
                              size={18}
                              color={day.rest_day ? 'rgba(214, 195, 181, 0.42)' : isDone ? '#4B2700' : WK_ACCENT_LIGHT}
                            />
                          </View>
                          <View style={{ flex: 1 }}>
          ...
          <GlassPanel style={styles.aboutCard}>
            <SectionLabel accent={WK_ACCENT_LIGHT}>About</SectionLabel>
