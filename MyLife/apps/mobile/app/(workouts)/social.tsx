...
export default function SocialProfileScreen() {
  ...
  const handleFollowToggle = useCallback(() => {
    if (profile.userId === LOCAL_SOCIAL_PROFILE_ID) {
      router.push('/(workouts)/(tabs)/settings' as never);
      return;
    }
  ...
  const renderPost = useCallback(
    ...
    [activeReactions, expandedComments, handleOpenProfile, handleReact, handleToggleComments, visibleFeed],
  );
      ...
      <WorkoutPhaseHeader
        ...
        right={(
          <Pressable style={styles.headerAction}>
            <MaterialSymbol name="more_horiz" size={18} color={WK_ACCENT_LIGHT} />
          </Pressable>
        )}
      />
                  ...
                  <View style={styles.heroActionRow}>
                    ...
                    <Pressable onPress={handleFollowToggle} style={styles.followWrap}>
                      {isFollowing || profile.userId === LOCAL_SOCIAL_PROFILE_ID ? (
                        ...
                        >
                          <MaterialSymbol name="add" size={16} color={WK_ACCENT_DARK} />
