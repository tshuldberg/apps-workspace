  ...
  const saveSupersetWorkout = () => {
    ...
    try {
      createWorkout(db, uuid(), {
        ...
        estimatedDuration: selectedSlots.reduce(
          (total, slot) => total + slot.sets * Math.max(slot.reps, 8) * 3 + slot.restAfter * slot.sets,
          0,
        ),
        isPremium: false,
      });
                ...
                {slot.exercise ? (
                  <>
                    ...
                    <View style={styles.chipRow}>
                      ...
                      >
                        <Chip label={`${slot.sets} sets`} selected accent={WK_ACCENT} />
                      </Pressable>
                      <Pressable
