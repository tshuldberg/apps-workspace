...
export default function DishBrowserScreen() {
  const router = useRouter();

  const [query, setQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<DishCategoryType | null>(null);
    ...
    try {
      const [dishResult, cuisineResult] = await Promise.all([
        searchDishes(query || '', {
          ...
          limit: 50,
          // status null: RLS scopes visibility; keeps a proposer's own
          // pending dish browsable (searchDishes now defaults to 'active').
          status: null,
        }),
        getCuisines(),
      ]);
      ...
      {showCuisines && cuisines.length > 0 && (
        ...
        >
          <FilterChip
            ...
            onPress={() => { setSelectedCuisine(null); setShowCuisines(false); }}
          />
          {cuisines.map((c) => (
            <FilterChip
              key={c}
