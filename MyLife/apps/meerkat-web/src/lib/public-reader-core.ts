// Public Reader state + copy core (Plan 19, Meerkat Public Social Layer -- P6,
// web twin). Byte-parity with apps/meerkat/app/(root)/data/public-reader-core.ts
// on all shared logic (copy consts, 5-state selection, verified-event grouping,
// signed local report persistence). The ONLY difference is where the post helpers
// are sourced: the web post primitives (deriveChannelPostTitle / isChannelPostEvent
// / isChannelPostRootEvent) live in ./meerkat-data, the mobile twin's in
// ./community-core. Everything below the imports is the same source.
//
