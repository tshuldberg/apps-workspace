// Auto-connect surface core (Plan 29 Phase 2-3, item 12 / AM11). Web twin of
// apps/meerkat/app/(root)/data/auto-connect-core.ts. The engine owns the real
// decision + execution (planAutoConnectRound / runAutoConnectJob in
// @mylife/sync); this is only the app-side seam that stores the opt-in flag,
// records the honest result of the last real round, and composes ONE shared
// foreground round out of the mailbox drain + the auto-connect session sync
// (+ gossip, once the engine seam lands). It fabricates nothing: every number
// shown comes from a real drain or a real runAutoConnectJob outcome.
//
// Honesty invariants:
//   - auto-connect is opt-in and OFF by default.
...
/** Toggle automatic dialing (opt-in only). */
...
export interface AutoConnectRoundInput {
   ...
   * engine wires gossip into a real session round, MeerkatProvider passes it
   * here so this ONE round composes drain + sessions + gossip. Absent = not yet.
   */
 ...
 * functions so it is unit-testable without the engine.
 */
