// Plan 30 Phase 4: the WEB twin of apps/meerkat/app/(root)/data/live-loop-core.ts.
//
// The channel view polls the SAME honest runForegroundDrain the manual Refresh
// button used to; there is no push, no fabricated status. All the timing logic
// lives here as pure functions + a scheduler with injected now/random/timer seams,
// so vitest can prove the cadence and the worst-case latency bound without a
// renderer. The React glue (visibilitychange + focus + relay gate) is the thin
// useChannelLiveLoop hook. Kept byte-identical to the mobile file (both surfaces
// share one cadence; scripts/check-meerkat-parity.mjs locks the constants).
