// Plan 25 WP-25I: the LiveKit SDK bridge for community rooms on web (twin of
// apps/meerkat/app/(root)/data/room-livekit-adapter.ts, over the browser
// livekit-client SDK instead of @livekit/react-native).
//
// UNVERIFIED - pending live LiveKit QA. Lazy-loads livekit-client via dynamic
// import and returns null when it is absent, so the room screen stays honestly
// unavailable. It translates REAL SDK callbacks into the pure room-view-core
// RoomEvent surface and adds no state of its own: phase/security/participants are
// derived by the core from these events only (NC-25.1 / NC-25.4), never fabricated
// here. There is no AudioSession on web (the browser owns audio output).
...
async function loadLiveKit(): Promise<LkModule | null> {
  try {
    const client = (await import('livekit-client')) as unknown as LkModule;
