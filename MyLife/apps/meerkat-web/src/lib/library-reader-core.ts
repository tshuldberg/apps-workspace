// Plan 38 Phase 6 (WEB): pure helpers for the sealed in-app epub/cbz readers.
//
...
// render chapter HTML inside a fully sandboxed, network-blocked iframe. Active
...
// fully sandboxed iframe (srcdoc + sandbox="") here, a network-blocked WebView
// (source={{ html }}) on mobile. Everything above this marker is byte-identical
// with the mobile source (CORE_TWINS bounded lock).

/** The iframe sandbox attribute for untrusted chapter HTML: fully sandboxed. */
export const EPUB_IFRAME_SANDBOX = '';

export interface SandboxDocument {
  /** Full HTML document to feed the iframe via `srcdoc`. */
  srcdoc: string;
  /** The value for the iframe `sandbox` attribute. */
  sandbox: string;
...
export function buildSandboxedChapter(bodyHtml: string): SandboxDocument {
  ...
  return { srcdoc, sandbox: EPUB_IFRAME_SANDBOX, csp: EPUB_CHAPTER_CSP };
}
