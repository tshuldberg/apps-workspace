// member-removal-view-core.ts (web twin of
// apps/meerkat/app/(root)/data/member-removal-view-core.ts). Plan 28 P4 web half:
// pure copy + count-to-phrase logic for the owner-only "Remove member" control.
// Kept native-free + byte-lockstep with the mobile source (parity guard) so the
// honest phrasing cannot drift from the settings surface.
//
...
export function describeMemberRemovalFailure(
  ...
  switch (result.reason) {
    ...
    default:
      ...
      // no-op (a null notice would render nothing).
