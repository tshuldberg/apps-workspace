// community-list-core.ts (web twin of apps/meerkat/app/(root)/data/community-list-core.ts).
// Pure helpers for the Communities LIST + settings role gating (Plan 31 Phase 5).
// Kept native-free so the unread aggregation and the owner/admin gating are
// testable under Node and cannot drift from the screens or the mobile source.
...
export interface CommunityAdminCaps {
  /** Owner-only rows (public reports, make public, owner review, Remove member). */
  isOwner: boolean;
