// invite-envelope-core.ts (web twin of
// apps/meerkat/app/(root)/data/invite-envelope-core.ts): pure builders for
// text-message-ready "envelopes" around raw Meerkat artifacts (a community
// invite link, a friend code). A raw meerkat:// link or MEER- code sent over
// a text message means nothing to a person who has never installed Meerkat, so
// every share surface wraps it in plain install + paste instructions. Honesty
// rules: an envelope never claims delivery, presence, or availability; the
// install step is OMITTED entirely when no install URL is configured (never a
// placeholder link); the friend-code envelope names the publish dependency
// instead of promising the code resolves. Kept lockstep with the mobile source
