...
export function buildHostedBoundaryItems(input: HostedBoundaryInput): HostedBoundaryItem[] {
  return [
    ...
    },
    {
      ...
      detail: 'This browser shows messages saved here. Community owner pays for always-on history when hosted history is connected.',
    },
    publicReachItem(
      'public_posts',
      'Public posts',
    ...
    ),
    {
      ...
      title: 'Large files',
      state: 'local_only',
      stateLabel: 'Browser storage',
      detail: `Files use this browser's local storage today (${input.localStorageLabel}). Paid hosted file storage is not connected; writes fail instead of pretending to upload.`,
    },
    {
      id: 'always_on_node',
