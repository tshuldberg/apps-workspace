...
// A fake browser WebRTC module: records what the adapter drives, and lets a test fire the
// connection-state, ICE, and track callbacks the real browser would.
function fakeModule() {
  ...
  const mod = {
    ...
    MediaStream: vi.fn(() => {
      const t: any[] = [];
      return { getTracks: () => t, getAudioTracks: () => t, getVideoTracks: () => t, addTrack: (x: any) => t.push(x) };
    }) as any,
...
describe('browser call media backend', () => {
  ...
  it('offer/answer round trip returns typed SDP from the browser', async () => {
  ...
  it('captures the remote MediaStream only from a real track event', async () => {
    const { mod, pc, stream } = fakeModule();
    const session = await loadCallMediaBackend({ module: mod as any })!.startPeerConnection({ callId: 'c1', media: 'video', role: 'offerer', localMicOn: true, localCamOn: true }) as any;
    // No remote stream until the peer connection delivers a track.
    expect(session.remoteStream()).toBeNull();
    expect(session.localStream()).toBe(stream);
    const remote = { getTracks: () => [], getAudioTracks: () => [], getVideoTracks: () => [] };
    pc.fire('track', { streams: [remote] });
    expect(session.remoteStream()).toBe(remote);
  });
