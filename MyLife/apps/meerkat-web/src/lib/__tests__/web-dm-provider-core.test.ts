/**
 * Plan 21 Phase 9 (WEB twin of apps/meerkat/app/__tests__/dm-provider-core.test.ts):
 * dm-provider-core.ts, the pure send/drain/link core wired into MeerkatProvider's
 * foreground drain (web has no background drain; foreground only).
 *
 * Verifies:
 ...
 *    MailboxRelayBackend (mirrors the P7 e2e test);
 *  - a foreground-drain parity test proving the SAME buildDmMailboxHandlers +
 *    runMailboxDrainJob path MeerkatProvider.runForegroundDrain composes applies a
 *    parked DM into dm_messages on the real web schema boot.
 */
...
describe('queueDmReceiptCore (send)', () => {
  it('parks a delivered receipt to the message author only on a real park', async () => {
    ...
    const { mergeDmEvents } = await import('../dm-core');
  ...
  it('skips a read receipt when the conversation has read receipts disabled', async () => {
    ...
    const event = createDmMessage(friend, { conversationId, body: 'hi', hlc: { wall: NOW, counter: 0 } });
    const { mergeDmEvents, setDmReadReceiptsEnabled } = await import('../dm-core');
    mergeDmEvents(db, conversationId, [event]);
...
describe('delivered-on-drain (Plan 21 Phase 10 item 2)', () => {
  it('emits a REAL delivered receipt when the drain applies an inbound message (recipient never opens the thread)', async () => {
    ...
    const { mergeDmEvents, setDmDelivery, getDmDelivery } = await import('../dm-core');
...
describe('foreground-drain parity: the SAME buildDmMailboxHandlers + runMailboxDrainJob path applies a parked DM', () => {
  // Web has NO background drain (foreground only); this is the web analogue of the
  // mobile background-sync parity test. It boots the REAL web schema (the same
  // ensureMeerkatTables + ensureSyncSchema the browser boot runs) + a real paired
  // device, then drives the exact handler set + drain job MeerkatProvider
  // .runForegroundDrain composes, proving a parked DM lands in dm_messages.
  it('runMailboxDrainJob with buildDmMailboxHandlers applies a parked DM message into dm_messages', async () => {
    const db = freshDb();
    const { ensureMeerkatTables, ensureSyncSchema } = await import('../schema');
    const { setSetting, RELAY_URL_SETTING_KEY } = await import('../meerkat-data');
    ...
    const event = createDmMessage(friend, { conversationId, body: 'foreground drain', hlc: { wall: NOW, counter: 0 } });
    const sealed = sealDmDirect({
      sender: friend, conversationId, events: [event],
      recipients: [{ deviceId: self.publicKey, dhPublicKey: self.dhPublicKey, pairSecret: secret }],
      now: NOW,
    ...
    const drain = await runMailboxDrainJob({
      nowMs: Date.parse(NOW),
      identity: self, backend, relayUrl: 'ws://relay',
      peers: [{ deviceId: friend.publicKey, pairSharedSecretHex: secret, revoked: false, isActive: true }],
      handlers: handlers as MailboxEnvelopeHandlers,
      waitForDrain: async () => {},
    });
