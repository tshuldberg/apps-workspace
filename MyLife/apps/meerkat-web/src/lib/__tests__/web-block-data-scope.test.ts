/**
 * Composition plan 2.1 sandbox rule (block-data-scope, WEB twin): block
...
import { BLOCK_REGISTRY } from '../block-registry-core';

const SRC = resolve(__dirname, '..', '..');
const rendererSource = readFileSync(resolve(SRC, 'ui/blocks/registry.tsx'), 'utf8');
const stackSource = readFileSync(resolve(SRC, 'ui/blocks/BlockStack.tsx'), 'utf8');
const queriesSource = readFileSync(resolve(SRC, 'lib/block-queries.ts'), 'utf8');
...
describe('renderer map import hygiene', () => {
  it('imports no data-layer module except the query seam and the registry core', () => {
    const dataImports = importPaths(rendererSource).filter((p) => p.includes('/lib/'));
    expect(dataImports.sort()).toEqual(['../../lib/block-queries', '../../lib/block-registry-core']);
  });
...
describe('BlockStack import hygiene', () => {
  it('imports only the pure cores and the renderer map', () => {
    const dataImports = importPaths(stackSource).filter((p) => p.includes('/lib/'));
