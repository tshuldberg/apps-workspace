// Plan 38 Phase 6 (WEB): a minimal, hardened, in-memory ZIP reader for the sealed
// epub/cbz readers. NO new dependency: inflate rides the platform
// `DecompressionStream('deflate-raw')`. epub/cbz bytes are ATTACKER-CONTROLLED
// community data, so every read is bounded by explicit caps (entry count, total
// uncompressed bytes, per-entry bytes) and nested archives are rejected. A cap
// breach throws; nothing is rendered from an over-budget archive.
...
function decodeName(b: Uint8Array, o: number, len: number, utf8: boolean): string {
  const slice = b.subarray(o, o + len);
  return new TextDecoder(utf8 ? 'utf-8' : 'utf-8').decode(slice);
}
...
export async function openZip(bytes: Uint8Array, caps: UnzipCaps): Promise<ZipArchive> {
  ...
  for (let i = 0; i < totalEntries; i += 1) {
    ...
    const name = decodeName(bytes, p + 46, nameLen, (flags & 0x800) !== 0);
...
/**
 * Inflate raw-deflate bytes with a hard output cap. We stop and throw the instant
 * the running output would exceed the cap, so a compression bomb can never
...
async function inflateBounded(
  ...
  for (;;) {
    ...
    if (value) {
      ...
      if (total > limit) {
        void reader.cancel();
        throw new Error('Inflated output exceeds size cap (possible zip bomb).');
      }
      chunks.push(value);
    }
  }
