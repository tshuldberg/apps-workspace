// Public-tier follows (Plan 39 P10, web twin of apps/meerkat/app/(root)/data/public-follows.ts).
// The device's OWN choice of which public personas and Commons topics to surface in its Following
// feed. Backed by cm_public_follows, which is DEVICE-LOCAL by policy (never replicates): a follow is
// a private viewing preference, not a shared or published signal. The host aggregates public follower
// counts from a separate registration, never from this table, so nothing here fabricates a count.
...
/** Follow a persona or topic. Idempotent (preserves created_at on a re-follow). No-op on a bad target. */
...
export function unfollowPublic(db: DatabaseAdapter, kind: FollowKind, targetId: string): void {
  const target = normalizeTarget(kind, targetId);
  db.execute(`DELETE FROM ${CM_PUBLIC_FOLLOWS_TABLE} WHERE follow_kind = ? AND target_id = ?`, [kind, target]);
}
