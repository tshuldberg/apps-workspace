// Plan 30 Phase 4: the WEB twin of apps/meerkat/app/(root)/components/chat/
// chat-kit-core.ts. Byte-identical pure logic (no React, no native, no provider):
// message grouping, day/unread dividers, the deterministic clock label, @mention
// detect/filter/insert, the signed-mention resolver, the body-highlight segmenter,
// the reaction-tap toggle, and the single-send latch. Kept in lockstep with the
// mobile file by scripts/check-meerkat-parity.mjs; edit BOTH in the same change.
...
/**
 * Structurally identical to the data layer's MessageReactionGroup. Declared here
 * so the kit imports no data module and stays purely prop-driven; the channel
 * view passes MessageReactionGroup[] straight in (structural typing).
 */
