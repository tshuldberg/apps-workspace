// Direct-message local store (Plan 21 Phase 9, WEB twin of the mobile
// apps/meerkat/app/(root)/data/dm-core.ts): the device-local SQLite store +
// helpers for 1:1 (and group) DMs. Byte-identical logic to the mobile file
// (both depend only on @mylife/db + @mylife/sync); check-meerkat-parity.mjs
// byte-locks the two so a future edit cannot silently diverge.
//
// CRITICAL: dm_ tables are LOCAL-ONLY, never replicate. `dm` is deliberately
// ABSENT from MEERKAT_SYNC_PREFIXES and MEERKAT_SYNC_POLICIES (meerkat-data.ts):
