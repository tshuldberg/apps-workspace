...
export interface DropboxBrokerSessionSource extends AccessTokenProvider {
  readonly credentialRef: string;
  revoke(): Promise<void>;
}

export interface DropboxBrokerSessionSourceOptions {
  client: DropboxBrokerSessionClient;
  vaultId: string;
  destinationId: string;
}

export interface DropboxWebSessionOptions {
...
export function createDropboxBrokerSessionSource(
  options: DropboxBrokerSessionSourceOptions,
): DropboxBrokerSessionSource {
...
export function createDropboxWebSessionSource(
  options: DropboxWebSessionOptions,
): DropboxBrokerSessionSource {
  const client = new OAuthBrokerClient({
    baseUrl: options.brokerBaseUrl,
    transport: options.transport ?? createDropboxBrowserHttpTransport(options.fetchImpl),
  ...
  return createDropboxBrokerSessionSource({
    client,
...
export function createDropboxBrowserHttpTransport(fetchImpl: typeof fetch = fetch): HttpTransport {
...
function requireSafeId(value: string, label: string): string {
  ...
  if (!SAFE_ID.test(normalized)) throw new Error(`Dropbox ${label} is invalid.`);
  return normalized;
}
