...
export interface GoogleDriveBrokerSessionSource extends AccessTokenProvider {
  readonly credentialRef: string;
  revoke(): Promise<void>;
}

export interface GoogleDriveBrokerSessionSourceOptions {
  client: GoogleDriveBrokerSessionClient;
  vaultId: string;
  destinationId: string;
}

export interface GoogleDriveWebSessionOptions {
...
export function createGoogleDriveBrokerSessionSource(
  options: GoogleDriveBrokerSessionSourceOptions,
): GoogleDriveBrokerSessionSource {
    ...
    async getAccessToken(operation) {
      try {
        ...
        if (error instanceof OAuthBrokerClientError
          && (error.code === 'auth_required' || error.code === 'vault_not_found')) {
          return null;
        }
        throw error;
      }
...
export function createGoogleDriveWebSessionSource(
  options: GoogleDriveWebSessionOptions,
): GoogleDriveBrokerSessionSource {
  const client = new OAuthBrokerClient({
    baseUrl: options.brokerBaseUrl,
    transport: options.transport ?? createBrowserHttpTransport(options.fetchImpl),
  ...
  return createGoogleDriveBrokerSessionSource({
    client,
...
export function createBrowserHttpTransport(fetchImpl: typeof fetch = fetch): HttpTransport {
...
function requireSafeId(value: string, label: string): string {
  ...
  if (!SAFE_ID.test(normalized)) throw new Error(`Google Drive ${label} is invalid.`);
  return normalized;
}
