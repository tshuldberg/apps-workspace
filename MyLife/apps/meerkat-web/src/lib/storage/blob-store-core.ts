...
//
// SOURCE OF TRUTH: this is a verbatim mirror of
//   apps/meerkat/app/(root)/data/blob-store-core.ts
// App Isolation (MyLife CLAUDE.md "App Isolation + Hub Inclusion"): meerkat-web
// must not deep-import files out of the apps/meerkat app boundary, so the pure
// logic is replicated here and kept in lockstep until a shared
// @mylife/meerkat-core package extracts it.
//
// Why this exists (Critical, mirrors file-save.ts's FileSaveAdapter seam):
//   The platform blob store imports a native/browser IO module that cannot
...
//   DatabaseAdapter directly. The platform blob store builds the real adapter and
//   delegates; tests inject a fake IO layer and an in-memory db.
//
...
/**
 * The single boundary to native blob-file IO. Wraps only the IO calls the
 * presence/removal logic needs, so tests inject a fake and never load a native
 * module.
 */
export interface BlobIoAdapter {
  /** Absolute on-disk path (or store key) for a content hash. */
