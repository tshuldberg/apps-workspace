// Device-local theme persistence for Meerkat (WEB TWIN). Pure CRUD over a
// DatabaseAdapter, the verbatim mirror of the canonical mobile store at
// apps/meerkat/app/(root)/theme/theme-store.ts. The DDL, column list, and every
// mk_themes / theme_* query string MUST stay identical to mobile (asserted by
// scripts/check-meerkat-parity.mjs). The platform glue differs by design: the
// web ThemeProvider drives this with the sql.js BrowserDatabaseAdapter plus a
// localStorage boot mirror; mobile uses expo-sqlite. It imports only @mylife/db
// types and the RN-free @mylife/meerkat-theme package.
//
