// Plan 32 T4.1 (web twin of mobile data/feed-view-core.ts): the pure view mapping
// for the content-first feed card. It sits BESIDE feed-core.ts and never touches
// its ranking logic (NC-5). It consumes the already-evaluated FeedItem (media /
// replyCount / reactions were populated in Plan 32 Phase 0) and turns it into the
// display + engagement model the card renders. Every engagement number derives
// from a verified FeedItem field (NC-1); nothing here fabricates a count. No React,
// so the mapping is unit-tested without a render harness. The logic mirrors the
// mobile twin exactly; only the meerkat-data import path differs.

import type { DatabaseAdapter } from '@mylife/db';
import { listChannelPostCards } from '../../lib/meerkat-data';
import type { FeedControlKey, FeedItem, FeedItemKind } from '../../lib/feed-core';

/** The quick-react emoji the feed engagement heart toggles (matches QUICK_REACTIONS[0]). */
export const HEART_EMOJI = '❤️';

/** Human label for a feed item kind (shown in the why-popover, not on the card). */
...
/** The source toggle that controls whether each kind appears (why-popover + filters). */
...
/**
 * The plain-text payload the Share affordance hands to the OS/web share sheet for a
...
/**
 * Resolve the signed root event id for a post-kind feed item so the heart's ADD
 * path can target it (`sendReaction` needs the root event id; the feed item only
 * carries postId). Uses the same read model that produced the card; returns null
 * when the post root is no longer resolvable.
 */
