...
export default function WorkoutHistoryScreen() {
  ...
  const handleDelete = useCallback(
    (entry: WorkoutHistoryEntry) => {
      Alert.alert(
        'Delete session?',
        `Delete "${entry.workoutTitle}" from ${formatDateLabel(entry.date)}? This cannot be undone.`,
        [
  ...
  // "Repeat" starts the same workout again when it still exists. If the
  // workout was deleted since, it rebuilds a one-off workout from what was
  // actually completed in this session so the user isn't stuck rebuilding
  // by hand (IMP-9). Shared with the Progress tab and Home dashboard so all
  // three surfaces resolve a repeat the same way.
      ...
      {state.error ? (
        ...
        <View style={styles.body}>
          <GlassPanel padding={24} style={styles.emptyCard}>
            <MaterialSymbol name="history" size={28} color={DW_ACCENT_LIGHT} />
              ...
              <View style={styles.stack}>
                {section.items.map((item) => (
                  <GlassPanel
                    ...
                    padding={18}
                    onPress={() =>
                      router.push(`/(root)/session?id=${item.sessionId}` as never)
                    }
                  >
                    <View style={styles.itemRow}>
                      ...
                      <Pressable
                        ...
                        accessibilityLabel={`Repeat ${item.workoutTitle}`}
                      >
                        <MaterialSymbol name="replay" size={18} color={DW_ACCENT_LIGHT} />
                      </Pressable>
                      <Pressable
                        onPress={() => handleDelete(item)}
                        style={styles.iconButton}
                        accessibilityRole="button"
                        accessibilityLabel={`Delete ${item.workoutTitle}`}
                      >
                        <MaterialSymbol name="delete" size={18} color="#FF9A9A" />
                      </Pressable>
                    </View>
                  </GlassPanel>
                ))}
