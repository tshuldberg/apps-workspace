        ...
        <>
          <View style={styles.rowBetween}>
            ...
            <Pressable style={styles.newButton} onPress={() => setShowForm(!showForm)}>
              <Text variant="label" color={DW_ON_ACCENT}>{showForm ? 'Cancel' : 'New Plan'}</Text>
            </Pressable>
          </View>

          {activeSub ? (
            <Card>
              <Text variant="label" color={DW_ACCENT}>Active Plan</Text>
          ...
          {showForm ? (
            <Card>
              ...
              <Pressable style={[styles.newButton, { marginTop: spacing.sm }]} onPress={handleCreate}>
                <Text variant="label" color={DW_ON_ACCENT}>Create Plan</Text>
              </Pressable>
            </Card>
          ) : null}
        </>
              ...
              <View style={{ gap: spacing.xs, alignItems: 'flex-end' }}>
                {!isActive ? (
                  <Pressable style={styles.subButton} onPress={() => handleSubscribe(item.id)}>
                    <Text variant="caption" color={DW_ON_ACCENT}>Subscribe</Text>
                  </Pressable>
                ) : (
                  <Text variant="caption" color={DW_ACCENT}>Active</Text>
                )}
...
const styles = StyleSheet.create({
  ...
  newButton: { borderRadius: 8, backgroundColor: DW_ACCENT, paddingHorizontal: spacing.md, paddingVertical: spacing.xs },
  ...
  subButton: { borderRadius: 6, backgroundColor: DW_ACCENT, paddingHorizontal: spacing.sm, paddingVertical: 4 },
