...
function difficultyColor(label: string): string {
  ...
  if (label === 'Advanced') return DW_ACCENT;
  return '#3B82F6';
}
    ...
    >
      ...
      {rows.length === 0 ? (
        ...
        rows.map((row) => (
          <Pressable
            key={row.plan.id}
            onPress={() => router.push(`/(root)/program/${row.plan.id}`)}
      ...
      <Pressable
        onPress={() => router.push('/(root)/program/create')}
        style={[styles.card, styles.createBtn]}
      >
        <Text variant="label" color={DW_ON_ACCENT}>+ Create Program</Text>
      </Pressable>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  cardActive: { borderLeftWidth: 3, borderLeftColor: DW_ACCENT },
  ...
  progressFill: { height: '100%', backgroundColor: DW_ACCENT },
  createBtn: { backgroundColor: DW_ACCENT, borderColor: DW_ACCENT, alignItems: 'center' },
});
