 ...
 *
 * Every networked dial in the app reads the relay URL through THIS helper, never
 * `getSetting(db, RELAY_URL_SETTING_KEY) || DEFAULT_RELAY_URL` directly. The raw
 * read cannot honor the per-device opt-out (a DB row, not encodable in the
 * build-time DEFAULT_RELAY_URL constant) nor the health gate, so it would dial a
 * free default the user turned off or one that is down -- both honesty
 * violations. This wrapper reads relay_url, default_relay_optout, and the cached
 * mk_relay_probe row, then delegates to the pure, tested decision in @mylife/sync.
 *
 * Behaviour (proven by packages/sync default-relay.test.ts + the Tier-B test):
 ...
 *  - '' when nothing is dialable (opted out, default down/unprobed, or an
 *    unconfigured build) -- so the caller's existing `.startsWith('ws')` guard
...
import { DEFAULT_RELAY_OPTOUT_KEY, getRelayProbe, getSetting, writeRelayProbe } from './db';
import { DEFAULT_RELAY_URL, RELAY_URL_SETTING_KEY } from './sync-core';
...
export function effectiveRelayUrl(db: DatabaseAdapter): string {
  ...
  // The probe only gates the default; consult the cached row for whichever URL
  // would actually be dialed (a user URL ignores its probe in the decision).
