...
// existing attachment + blob pipeline, inheriting its per-blob hash verification
// and replication for free. The 64 KB preview-JSON cap is NOT inherited from the
// blob store (putLocal does not cap): it is enforced HERE at encode AND parse. A
// malformed or oversized preview parses to null and the UI degrades to a plain
// file chip; nothing here throws, fetches on receive, or trusts a remote URL on
// the receiver.
//
// This file is a BYTE-IDENTICAL twin with apps/meerkat-web/src/lib/link-preview.ts
