...
import {
  ONEDRIVE_DISCOVERY,
  ONEDRIVE_NATIVE_CREDENTIAL_PREFIX,
  ONEDRIVE_NATIVE_SCOPES,
  ONEDRIVE_REFRESH_TOKEN_KEY_PREFIX,
  createOneDriveNativeTokenSource,
...
} from '../onedrive-source';

const NOW_MS = Date.parse('2026-07-14T12:00:00.000Z');

describe('OneDrive native OAuth token source', () => {
  it('returns honest null when native OAuth or founder registration is unavailable', async () => {
    await expect(createOneDriveNativeTokenSource({
      clientId: 'registered-client',
      redirectUri: 'com.mylife.meerkat:/oauth/onedrive',
  ...
  it('uses Microsoft PKCE endpoints and keeps refresh tokens in SecureStore custody', async () => {
    const auth = fakeAuth();
    const store = new MemoryStore();
    const source = await createOneDriveNativeTokenSource({
      clientId: 'registered-client',
      redirectUri: 'com.mylife.meerkat:/oauth/onedrive',
      destinationId: 'destination-1',
    ...
    expect(result).toEqual({
      credentialRef: `${ONEDRIVE_NATIVE_CREDENTIAL_PREFIX}destination-1`,
      accountHint: null,
    });
    expect(JSON.stringify(result)).not.toContain('refresh-token-canary');
    expect(auth.discovery).toEqual([ONEDRIVE_DISCOVERY]);
    expect(auth.requestConfigs[0]).toMatchObject({ scopes: [...ONEDRIVE_NATIVE_SCOPES], usePKCE: true });
    expect(store.values.get(`${ONEDRIVE_REFRESH_TOKEN_KEY_PREFIX}destination-1`)).toBe('refresh-token-canary');
  ...
  it('contains no SQLite, AsyncStorage, or browser token storage path', () => {
    const source = readFileSync(resolve(__dirname, '..', 'onedrive-source.ts'), 'utf8');
