// Public persona profile reader (Plan 39 P10, screen S8). A profile is assembled
// PURELY from a persona's signed public posts -- there is nothing else to show, by
// architecture (no device, location, or private-community data exists for a public
// persona). This pages every configured Commons topic, filters to the persona's
// authored posts, and unions them newest-first; every post is dual-verified against
// the descriptor-pinned node key by fetchPublicPage (fail-closed). Twin of
// apps/meerkat-web/src/lib/public-profile.ts.
//
// HONESTY: only real, dual-signed posts by THIS persona appear; the count is the
// real number loaded. No fabricated "member since" date, follower, or post totals
// (NC-P6) -- we surface only what the persona actually signed and the node served.
...
export interface PersonaProfile {
  ...
  /** The persona's top-level public posts (newest first), dual-verified. */
  posts: AcceptedPublicPost[];
  /** The channelId each post was found in, keyed by postId (for the "in {topic}" line). */
...
/**
 * Load a persona's public post history across every configured topic. Best-effort:
 * a topic that fails to fetch is skipped, but if EVERY topic is unreachable (and
 * none succeeded) the result is an honest `unreachable`. Only top-level posts
 * (not replies) form the profile timeline, matching the mockup.
 */
export async function loadPersonaProfile(
  ...
  for (const source of config.topics) {
    ...
    if (!page.ok) continue; // skip an unreachable/not-found topic; try the rest
    anySucceeded = true;
    for (const accepted of page.publicPosts) {
      if (accepted.post.personaPubkey.toLowerCase() !== wantKey) continue;
      if (accepted.post.parentPostId !== null) continue; // timeline is top-level posts only
