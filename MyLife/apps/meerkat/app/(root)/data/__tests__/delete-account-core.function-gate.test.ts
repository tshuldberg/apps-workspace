...
describe('runDeleteMyData function quality gate', () => {
  it('fails before any destructive callback when remote deletion fails', async () => {
  ...
  it('passes deterministic secret-count fuzz invariants', async () => {
    await runDeterministicFuzz({
      label: 'runDeleteMyData fuzz', iterations: 50, seed: 42,
  ...
  it('stays within linear complexity slope budget', async () => {
    await assertComplexitySlope({
      label: 'runDeleteMyData', sizes: [50, 100, 200], expected: 'linear', setup: dependencies,
  ...
  it('stays within memory budget under repeated calls', async () => {
    await assertMemoryBudget({
      label: 'runDeleteMyData', repeats: 20, maxHeapDeltaBytes: 8 * 1024 * 1024,
