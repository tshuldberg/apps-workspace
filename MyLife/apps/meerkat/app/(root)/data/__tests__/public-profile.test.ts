...
function fakeDb(): DatabaseAdapter {
  return { execute() {}, query<T = Record<string, unknown>>(): T[] { return [] as unknown as T[]; }, transaction(fn: () => void) { fn(); } };
}
...
/** Route each topic's /page GET to its own post set (keyed by publicationId in the url). */
...
describe('publicPostCountLabel', () => {
  it('singular/plural', () => {
    expect(publicPostCountLabel(0)).toBe('0 public posts');
    expect(publicPostCountLabel(1)).toBe('1 public post');
    expect(publicPostCountLabel(5)).toBe('5 public posts');
  });
});

describe('loadPersonaProfile (real dual-verified, honest)', () => {
  it('not_configured with no node/topics', async () => {
    const r = await loadPersonaProfile(fakeDb(), { nodeUrl: '', topics: [] }, a.publicKeyHex, topicFetch({}));
  ...
  it('unions the persona top-level posts across topics, newest first, excluding replies + other authors', async () => {
    ...
    const r = await loadPersonaProfile(fakeDb(), FEED, a.publicKeyHex, topicFetch({
      'pub-commons': [aCommons, aReply, bCommons],
      'pub-gaming': [aGaming],
    }));
    expect(r.ok).toBe(true);
    if (r.ok) {
      // Alice's top-level posts only, newest first; no reply, no bob.
      expect(r.profile.posts.map((p) => p.post.body)).toEqual(['alice in gaming', 'alice in commons']);
      expect(r.profile.channelByPostId[aGaming.post.postId]).toBe('gaming');
    }
  });

  it('best-effort: a failing topic is skipped; all-failing is unreachable', async () => {
    const aCommons = accepted(a, 'commons', 'pub-commons', 'alice in commons', null, '2026-07-07T00:01:00.000Z', 0);
    // gaming fails, commons succeeds -> still ok with the commons post.
    ...
    if (partial.ok) expect(partial.profile.posts).toHaveLength(1);
    // both fail -> unreachable.
