// capability-status.ts: the single source of truth for "What works today" (Plan
// 31 Phase 4, design decision 8). One typed list of capabilities with a live |
// partial | pending status and an honest one-liner each. Honesty by construction:
// the DM and default-server statuses DERIVE from the real flags/substrate
// (DM_MESSAGES_SURFACE_AVAILABLE, DEFAULT_RELAY_URL), so an entry can never claim
// "live" for a capability whose flag is off. Web twin:
// apps/meerkat-web/src/lib/capability-status.ts (kept in lockstep).

import { loadNativeNearbyModule } from '@mylife/meerkat-native-transport';
import { DM_MESSAGES_SURFACE_AVAILABLE } from './share-route';
import { DEFAULT_RELAY_URL } from './sync-core';
...
export interface CapabilityEntry {
  ...
  /** One honest line. Never implies a capability that is not really built. */
  line: string;
}
...
export function getCapabilityStatus(): CapabilityEntry[] {
  const dmLive = DM_MESSAGES_SURFACE_AVAILABLE;
  const hasDefaultServer = DEFAULT_RELAY_URL.length > 0;
  // Plan 53: derives from the REAL nearby substrate, so this build can never
  // claim in-person adding while the native module is absent (Expo Go).
  const nearbyAvailable = loadNativeNearbyModule() !== null;
  return [
    {
      id: 'community_chat',
    ...
    },
    {
      ...
      status: nearbyAvailable ? 'live' : 'pending',
      line: nearbyAvailable
        ? 'Add a friend, or pair your own device, by holding two phones near each other and confirming the same five emoji on both screens. No internet and no connection server are used; nearby device sync pauses while the screen is searching.'
        : 'Adding in person needs the full Meerkat app on both phones; this build does not include the nearby radio module.',
    },
    {
      id: 'calls',
    ...
    },
    {
      ...
      status: 'partial',
      line: 'On a dev build, a photo library can show located photos on an offline map after you download a map pack once. Only photos whose location you chose to keep appear, and your location and viewing are never sent anywhere. The web app has no map.',
    },
  ];
}
