// Plan 38 amendment C.6 (MOBILE): the pure photo-timeline + map-points view
// logic. This is the MOBILE source of truth; apps/meerkat-web/src/lib/
// photo-timeline-core.ts is a byte-identical twin (only this header differs).
//
...
// ONLY the items that carry consented GPS coordinates (D.7: an item has latitude/
// longitude in its signed metadata only when its contributor kept locations on at
// ingest; every other photo had its GPS stripped from the bytes before sealing).
//
// Pure + node-only (no React, no db, no native map): the grouping and the GPS
// consent filter are unit-tested without a render harness or a dev build.
