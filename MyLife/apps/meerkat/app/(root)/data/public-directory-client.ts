// Public directory probe client (Plan 19, Meerkat Public Social Layer -- P5a).
//
...
//
// RN-safe: the @mylife/sync directory client uses the global WebSocket; the app
// injects the React Native WebSocket via webSocketImpl, exactly like
// remote-share.ts injects it for content-host discovery.
...
import { PUBLIC_DIRECTORY_URL_SETTING } from './sync-core';
...
export interface ProbePublicDirectoryInput {
  ...
  searchTerms?: string[];
  /** RN global WebSocket, injected by the app (see remote-share.ts). */
 ...
 * and dual-verifies every public post against the pinned node key, fail-closed -- the
 * node is never trusted), advances the cursor (row-only, device_local) to the newest
 * VERIFIED item (event or post: a posts-only page still advances), and returns the new
 * verified items for the reader to append. A second call with no newer content returns
 * [] and does not advance, so re-paging never duplicates.
 */
