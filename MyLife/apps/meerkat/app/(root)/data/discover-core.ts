// Discover screen state + copy core (Plan 19, Meerkat Public Social Layer -- P5b).
//
// Pure, Node-testable helpers behind the Discover tab. They own the verbatim
...
// the screen runs probePublicDirectory / getDirectoryCacheEntries and feeds the
// results in here, so the state machine is testable without React Native.
...
// Verbatim section 7.1 copy. Exported so the screen and the tests read the same
// strings (the screen must render these exact words).
