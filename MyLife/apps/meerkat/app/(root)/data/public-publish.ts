...
//
// RN-safe: every side-effecting seam (fetch, WebSocket, secure random, the engine
// fns, the channel-event read) is injected through PublishDeps with a real
// default, so the screen runs the real path and the e2e drives the SAME function
// against live servers (TC-8).
...
import { listChannelMessageEvents } from './community-core';
