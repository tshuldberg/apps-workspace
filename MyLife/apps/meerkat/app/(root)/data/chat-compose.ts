...
/**
 * Plan 30 T0.5/T0.6: pure, provider-agnostic builders for outgoing channel
 * events. ChatProvider delegates event construction here so the v1/v2 selection
 * and reaction shape are unit-testable without a React harness (the mobile test
 * env is node-only). No side effects: callers persist + park the returned event.
 */
