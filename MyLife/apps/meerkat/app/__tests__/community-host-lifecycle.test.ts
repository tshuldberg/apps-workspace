/**
 * Plan 57 W2/W3: community server (host) lifecycle over the descriptor chain.
 *
 * Proves against a wire-accurate fake node (healthz + challenge + publish +
 * append routes, real signature verification on append):
 *  1. setCommunityHost attaches ONLY after a real healthz answer AND an
 *     accepted publish; the committed descriptor carries the host and the
 *     built snapshot records persist as the incremental baseline.
 *  2. An unreachable server attaches NOTHING (fail-closed, reason verbatim).
 *  3. A refused publish attaches NOTHING and surfaces the node reason.
 *  4. Non-owner and malformed-url calls are rejected before any request.
 *  5. clearCommunityHost commits the exit locally FIRST; a dead node cannot
 *     block its own firing (exitRepublished false, host still removed).
 *  6. appendEventToCommunityHost mirrors a recorded event to the host as a
 *     REAL SealedTailEntry that verifies against the descriptor roster.
 *  7. publishCommunitySnapshotsToHost pushes refreshed snapshots to the host
 *     and is an honest no-op without one.
 */
...
describe('normalizeCommunityHostUrl', () => {
...
describe('setCommunityHost', () => {
  it('attaches after healthz + accepted publish; descriptor carries the host', async () => {
    ...
    // The node received the REVISED descriptor (hosts populated, revision bumped).
    ...
    // Snapshot baseline persisted for later incremental publishes.
    const rows = db.adapter.query<{ channel_id: string }>(
      'SELECT channel_id FROM cm_snapshots WHERE community_id = ?', [communityId],
    );
    expect(rows.map((r) => r.channel_id)).toEqual([CHANNEL]);
  });
  ...
  it('attaches nothing when the health check fails', async () => {
    ...
    expect(node.state.publishes).toHaveLength(0);
  });
...
describe('clearCommunityHost', () => {
  it('commits the exit locally first; a dead node cannot block its firing', async () => {
    ...
    // The node dies; firing it must still succeed locally.
...
describe('appendEventToCommunityHost', () => {
  ...
  it('mirrors a recorded event as a real SealedTailEntry the roster verifies', async () => {
    ...
    // The fake node ALREADY verified the outer signature against the roster;
    // assert the captured entry's scope + author on top.
...
describe('durable join queue over the host (Plan 57 W4)', () => {
  it('joiner parks a REAL sealed request on the invite host; the asleep owner later drains it, mints a real grant, and the joiner lands in the roster', async () => {
    ...
    // A brand-new joiner builds + parks a real sealed request from the invite.
    ...
    // The owner "wakes" and drains its request box through the REAL handlers;
    // the grant parks back onto the node's durable box (captured token).
    ...
    // The owner's descriptor now LISTS the joiner (a real signed revision), and
    // the request box is empty after the ack.
    ...
    // The grant sits durably in the joiner's box on the node.
...
describe('publishCommunitySnapshotsToHost', () => {
  it('is an honest no-op without a host', async () => {
