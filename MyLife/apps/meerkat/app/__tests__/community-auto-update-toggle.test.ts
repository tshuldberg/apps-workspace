...
// in-memory database. Native twin of the web test of the same name.

import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { createInMemoryTestDatabase, type InMemoryTestDatabase } from '@mylife/db';
import { ensureMeerkatTables, getAutoUpdate, setAutoUpdate } from '../(root)/data/db';
...
describe('community feed P4: per-community auto-update toggle', () => {
  it('defaults OFF, persists ON/OFF row-only, and is scoped per community', () => {
    ...
    // Turn it back off; the row persists the off value.
