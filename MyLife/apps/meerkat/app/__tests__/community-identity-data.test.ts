// Plan 38 Phase 1b: community identity data layer. publish/get round trips with a
// real owner identity; a non-owner publish is REFUSED; a non-owner (forged) event
// resolves to null (renders nothing); the per-community theme mode round-trips and
// defaults to 'community'. Mirrors the cm_profiles write/read flow.
...
describe('publishCommunityIdentity / getCommunityIdentity round trip', () => {
...
describe('owner enforcement', () => {
  ...
  it('resolves a non-owner (forged) identity event to null (renders nothing)', () => {
    // A stranger signs a well-formed event and it is inserted directly, bypassing
    // the publish owner-check. Read-time resolution binds to the descriptor owner,
    // so the forged event never wins.
...
describe('per-community theme mode (device-local)', () => {
