/**
 * Plan 52 P1/P2 app core: the attestation exchange, auto-alignment, and the
...
import { listCommunityProfileEvents } from '../(root)/data/community-core';
import {
  alignPersonIdentity,
...
// ---------------------------------------------------------------------------
// Plan 53 P3: announcing an existing person to a newly added friend.
// ---------------------------------------------------------------------------

describe('announcePersonToDmPeer (plan 53 P3)', () => {
  let self: Device;
  let friend: Device;
  let mailbox: FakeMailbox;
  const SHARED = 'd'.repeat(64);

  function friendDeps(ownIds: string[] = []): PersonCeremonyDeps {
    return {
      db: self.db,
      identity: self.identity,
      listOwnDeviceIds: () => ownIds,
      listDmPeerIds: () => [friend.identity.publicKey],
      resolvePairedDevice: (deviceId) => (
        deviceId === friend.identity.publicKey
          ? { dhPublicKey: friend.identity.dhPublicKey, sharedSecretHex: SHARED }
          : null
      ),
      parkEnvelope: async (token, envelope) => mailbox.park(token, envelope),
    };
  ...
  async function friendQueue(): Promise<MailboxEnvelope[]> {
    const { deriveMailboxToken } = await import('@mylife/sync');
    return mailbox.take(deriveMailboxToken(SHARED, friend.identity.publicKey, Date.now()));
  }

  beforeEach(() => {
    configureSyncSecretStore(createInMemorySyncSecretStore());
    mailbox = new FakeMailbox();
    self = makeDevice('Solo Person');
    friend = makeDevice('New Friend');
  });

  it('announces an existing person to a new friend, and the friend links it', async () => {
    // The person group formed BEFORE this friend existed, so no dm-peer
    // context announce exists yet. That is exactly the in-person-add case.
    const genesis = await proposePersonGroupRevision({ ...friendDeps(), listDmPeerIds: () => [] }, {});
    expect(genesis.ok && genesis.assembled).toBe(true);

    const parked = await announcePersonToDmPeer(friendDeps(), friend.identity.publicKey);
    expect(parked).toBe(1);

    // The friend applies the parked announce and now renders the person's
    // device under the DM scope, from the verified proof only.
    const envelopes = await friendQueue();
    expect(envelopes).toHaveLength(1);
    const handlers = buildPersonGroupMailboxHandlers({
      db: friend.db,
      identity: friend.identity,
      listOwnDeviceIds: () => [],
      resolvePairedDevice: () => null,
      parkEnvelope: async () => false,
    });
    const outcome = await applyMailboxEnvelope(
      friend.identity,
      encodeMailboxEnvelope(envelopes[0]!),
      handlers,
    );
    expect(outcome.kind).not.toBe('rejected');
    const links = readPersonLinks(friend.db, DM_PERSON_LINKS_SCOPE);
    expect(links.map((l) => l.device_id)).toEqual([self.identity.publicKey]);
  });

  it('is an honest no-op without a person group', async () => {
    expect(await announcePersonToDmPeer(friendDeps(), friend.identity.publicKey)).toBe(0);
    expect(await friendQueue()).toHaveLength(0);
  });

  it('refuses an own-device-linked peer, protecting the linking proposal flow', async () => {
    const genesis = await proposePersonGroupRevision({ ...friendDeps(), listDmPeerIds: () => [] }, {});
    expect(genesis.ok).toBe(true);
    const parked = await announcePersonToDmPeer(
      friendDeps([friend.identity.publicKey]),
  ...
  it('a repeat announce parks nothing new', async () => {
    await proposePersonGroupRevision({ ...friendDeps(), listDmPeerIds: () => [] }, {});
    expect(await announcePersonToDmPeer(friendDeps(), friend.identity.publicKey)).toBe(1);
    expect(await announcePersonToDmPeer(friendDeps(), friend.identity.publicKey)).toBe(0);
    expect(await friendQueue()).toHaveLength(1);
  });
});
