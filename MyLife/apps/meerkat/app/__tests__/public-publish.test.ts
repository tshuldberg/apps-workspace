...
import { requestPublicJoin } from '../(root)/data/public-join-client';
...
// vi.fn(impl) infers an overloaded fetch type whose overload variance does not
// match the source's `typeof fetch` param under the mobile (RN) lib resolution.
// Wrap once so the spy is assignable to a fetch param AND keeps Mock assertions.
function fetchMock(impl: typeof fetch): typeof fetch & Mock {
  return vi.fn(impl) as unknown as typeof fetch & Mock;
}

function seedChannel(adapter: ReturnType<typeof createInMemoryTestDatabase>['adapter'], owner: DeviceIdentity): void {
...
describe('unpublishPublicly (Plan 19 FF1: terminal takedown of a REVISED publication)', () => {
  it('reconstructs a revision >= 2 row and signs an owner takedown (no cannot_reconstruct)', async () => {
    ...
    const result = await unpublishPublicly(
      {
        fetchFn: fetchMock(fetchStub({ [host]: 200 })),
        announceHost,
...
function publishWithArchive(
  adapter: ReturnType<typeof createInMemoryTestDatabase>['adapter'],
...
describe('durable-archive publish (Plan 19 P9.3e)', () => {
  ...
  describe('archivePublishResultView (honest states; no archive Success terminal in this build)', () => {
    ...
    it('renders the honest QUEUED note (never the archived terminal) for a pending archive', () => {
      ...
      // The base publish view still honestly reports the content is live + readable.
  ...
  describe('archive persistence + rights round-trip', () => {
    ...
    it('refreshes and cancels managed jobs with owner auth and mirrors only server state', async () => {
      ...
      const statusFetch = fetchMock(async (_url, init) => {
      ...
      const cancelFetch = fetchMock(async (_url, init) => {
    ...
    it('a no-archive publish writes null rights_json + no archive job (legacy behavior preserved)', async () => {
      ...
      const result = await publishChannelPublicly(
        deterministicDeps({ fetchFn: fetchStub({ 'https://h1.example': 200 }) }),
    ...
    it('a rev1 rights-bearing publish reconstructs + unpublishes (no cannot_reconstruct); the takedown is owner-valid', async () => {
      ...
      const un = await unpublishPublicly(
        { fetchFn: fetchMock(fetchStub({ 'https://h1.example': 200 })), announceHost, now: () => '2026-06-28T01:00:00.000Z' },
    ...
    it('reconstruction reads cm_publications.rights_json, NOT the device_local mirror: takedown survives deleting cm_publication_rights (Q2)', async () => {
      ...
      seedChannel(db.adapter, owner);
      const pub = await publishWithArchive(db.adapter, owner);
      // Simulate a second device / local reset: the device_local rights mirror is gone.
      ...
      const un = await unpublishPublicly(
        { fetchFn: fetchMock(fetchStub({ 'https://h1.example': 200 })), announceHost: vi.fn(async () => {}), now: () => '2026-06-28T02:00:00.000Z' },
    ...
    it('empty provenance round-trips: a rev1 publish with provenance "" still reconstructs + unpublishes', async () => {
      ...
      const un = await unpublishPublicly(
        { fetchFn: fetchMock(fetchStub({ 'https://h1.example': 200 })), announceHost: vi.fn(async () => {}), now: () => '2026-06-28T03:00:00.000Z' },
        { db: db.adapter, identity: owner, publicationId: pub.publicationId!, directoryUrl: '' },
      );
      expect(un.reason).not.toBe('cannot_reconstruct');
      expect(un.ok).toBe(true);
...
describe('publishChannelPublicly (Plan 19 P7a orchestrator)', () => {
  ...
  it('publishes to all hosts -> Success, announces, and persists the signed descriptor', async () => {
    ...
    expect(result.hosts.every((h) => h.accepted)).toBe(true);
    ...
    const rows = db.adapter.query<{ publication_id: string; status: string; signature_hex: string }>(
      'SELECT publication_id, status, signature_hex FROM cm_publications',
    );
  ...
  it('returns the rate-limited Error when every host 429s, and persists nothing', async () => {
    const db = createInMemoryTestDatabase();
    const owner = generateDeviceIdentity('Owner');
    seedChannel(db.adapter, owner);
    const fetchFn = fetchStub({ 'https://h1.example': 429 });
    ...
    expect(result.message).toBe(PUBLISH_COPY.errorRateLimited);
    expect(result.link).toBeNull();
    expect(db.adapter.query('SELECT publication_id FROM cm_publications')).toHaveLength(0);
    db.close();
  });

  it('returns the no-host Error when the host transport fails', async () => {
    const db = createInMemoryTestDatabase();
    const owner = generateDeviceIdentity('Owner');
    seedChannel(db.adapter, owner);
    const fetchFn = (async () => {
  ...
  it('exposes the hosted price from billing-config, never hardcoded in copy', () => {
    ...
    // The DEFAULT deps wire the real engine fns (no fakes).
    expect(typeof DEFAULT_PUBLISH_DEPS.buildPublicSnapshot).toBe('function');
  });
});

describe('publish-sheet (Plan 19 P7b) seams', () => {
  /** A fetch stub mapping a /healthz URL prefix -> HTTP status. */
  ...
  describe('probeServingHost (REAL reachability, fail-closed)', () => {
    it('is reachable only on a real HTTP 200 from GET {host}/healthz', async () => {
      const fetchFn = fetchMock(healthStub({ 'https://up.example': 200 }));
    ...
    it('rejects an empty or non-http URL without a network call', async () => {
      const fetchFn = fetchMock(healthStub({}));
...
describe('FF3 public-join grant emission (Plan 19 P9, app half)', () => {
...
describe('Plan 19 FF3 (app half, PART 1): public-join owner review queue', () => {
   ...
   * SyncProvider.createCommunity + publishChannelPublicly are actually wired.
   */
  ...
  it('drops a request when the humanity redeem fails (single-use / no service = no row)', async () => {
    ...
    // A replayed token: the service reports already_spent -> DROP, record NO row.
    ...
    // No service configured (no redeem client) -> fail closed, no row.
