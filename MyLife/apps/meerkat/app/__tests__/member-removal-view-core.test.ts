// Plan 28 P4: pure copy + count-to-phrase logic for the owner-only Remove control.
// Every phrase is derived ONLY from the real RemoveCommunityMemberResult counts;
// these tests lock the epoch-boundary honesty (NC-1) and the "never claim a park
// or republish that returned false" rule.
...
} from '../(root)/data/member-removal-view-core';
...
describe('describeMemberRemovalSuccess', () => {
  it('reports no other members when there are no keyed survivors', () => {
    const msg = describeMemberRemovalSuccess(ok({ survivorsToNotify: 0, envelopesParked: 0 }), 'Bo');
    expect(msg).toBe('Removed Bo. No other members needed the update.');
  });
  ...
  it('uses the singular member noun when exactly one survivor was queued', () => {
    const msg = describeMemberRemovalSuccess(ok({ survivorsToNotify: 1, envelopesParked: 1 }), 'Bo');
    expect(msg).toContain('1 remaining member;');
  });
...
describe('describeMemberRemovalFailure', () => {
  ...
  it('surfaces an out-of-union reason honestly instead of a silent no-op', () => {
    const msg = describeMemberRemovalFailure({ ok: false, reason: 'bogus' as never }, 'Bo');
    expect(msg).toBe('Could not remove this member.');
  });
});
