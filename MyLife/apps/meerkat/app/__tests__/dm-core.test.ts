// Plan 21 Phase 3: the device-local DM store (dm-core.ts).
//
// dm_ tables are LOCAL-ONLY by deliberate omission from MEERKAT_SYNC_PREFIXES /
// MEERKAT_SYNC_POLICIES (sync-core.ts). DM confidentiality comes from the
// mailbox seal in flight + the platform secure store at rest, not from session
// replication. The last describe block proves that omission actually holds by
// running a dm_ row through the real inbound-policy engine.
...
} from '@mylife/sync/src/protocol/sync-session';
import {
...
describe('ensureDmTables', () => {
  it('is idempotent and creates the local-only dm_ schema', () => {
    ...
    // dm_own_devices exists with the expected columns.
    ...
    // dm_participants carries identity_anchor + is_self.
...
describe('participants + own devices', () => {
  beforeEach(() => ensureDmTables(db.adapter));
...
describe('supersedes resolution', () => {
  ...
  it('an edit event replaces the displayed body in the same slot', () => {
    ...
    // The edit keeps the original message's SLOT (ordering position), while the
    // displayed event is the edit event itself (mirrors resolveChannelMessages).
...
describe('dm_delivery transitions', () => {
  beforeEach(() => ensureDmTables(db.adapter));
...
describe('unread math', () => {
  ...
  it('counts peer messages after last-read, excludes own messages, and respects the HLC boundary', () => {
    ...
    expect(getDmUnreadCount(db.adapter, 'c1', self.publicKey)).toBe(3); // no read state yet: all non-self

    setDmReadState(db.adapter, 'c1', '2026-06-30T00:01:00.000Z', 5); // boundary = m3's own HLC

    // m1 is before the boundary (excluded); m3 is exactly at the boundary (excluded,
    // not strictly greater); m4 is after (included); m2 is self-authored (excluded).
  ...
  it('read receipts enabled defaults true and toggles', () => {
    ...
    // toggling receipts does not disturb the last-read HLC
...
describe('LOCAL-ONLY invariant: dm_ never replicates', () => {
  it('rejects a dm_ row offered to the real inbound-policy engine (applyReceivedDocumentChanges)', () => {
    ensureDmTables(db.adapter);
    createSyncTables(db.adapter);

    const local = generateDeviceIdentity('Local Device');
    const peer = generateDeviceIdentity('Peer Device');
    ...
    // dm_conversations resolves to NO module: the real MEERKAT_SYNC_PREFIXES map
    // has no 'dm' entry, unlike mp_/cm_/sync_workspace_keys.
    expect(changeTracker.resolveModule('dm_conversations')).toBeNull();

    const options = {
      db: db.adapter,
      identity: { publicKey: local.publicKey },
      pairedDevices: [{ deviceId: peer.publicKey }], // paired: authorization is not the blocker here
      changeTracker,
    ...
    const change = {
      ...
      rowId: 'smuggled-conversation',
      operation: 'INSERT',
      data: { id: 'smuggled-conversation', kind: 'direct', updated_at: '2026-06-30T00:00:00.000Z' },
    };
    ...
    const applied = applyReceivedDocumentChanges(
      ...
      MEERKAT_SYNC_MODULE_ID,
      [change],
      { remoteDeviceId: peer.publicKey, sessionId: 'sess-1' },
    );

    expect(applied).toBe(0);
    expect(getDmConversation(db.adapter, 'smuggled-conversation')).toBeNull();
    expect(db.adapter.query('SELECT * FROM dm_conversations')).toHaveLength(0);
  });
});
