...
import { ensureCommunityTables, getCommunityIdentity, getCommunityLayoutEvent } from '../(root)/data/community-core';
import { ensureMeerkatTables, getSetting } from '../(root)/data/db';
import { createCommunityFromTemplate } from '../(root)/data/community-template-commit';
import { buildOnboardingExperience, ONBOARDING_EXPERIENCES, type ExperienceId } from '../(root)/data/onboarding-experience-core';
...
function commit(adapter: DatabaseAdapter, id: string): string {
  ...
  return createCommunityFromTemplate(adapter, identity, {
    name: draft.name, template: draft.template, adoptThemeBlob: draft.themeBlob,
    genesisChannels: draft.channels, layoutBlob: draft.layoutBlob, completeOnboarding: true, localLayoutDefault: { profile: 'mobile', choice: id as ExperienceId },
  }).communityId;
}
