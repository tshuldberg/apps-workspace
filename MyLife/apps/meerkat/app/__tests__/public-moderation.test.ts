// Plan 19 P8b: owner moderation core (the Public reports owner-fetch + the real
// Unpublish orchestration). Drives the SHIPPING app functions against stubbed
// fetch/announce seams. publishChannelPublicly persists a real genesis + snapshot;
// unpublishPublicly then re-registers the signed UNPUBLISHED revision (with the
// stored pieces) and re-announces it under the category rid, exactly mirroring the
// P8a relay e2e wire.
...
import { ensureCommunityTables, insertMessageRow } from '../(root)/data/community-core';
import {
  PUBLIC_REPORTS_COPY,
...
describe('P8b owner Public-reports fetch (owner-signed GET)', () => {
  it('sends owner-signed headers, sorts priority-first, and verifies the signature', async () => {
    ...
    const fetchFn = (async (url: string | URL, init?: { headers?: Record<string, string> }) => {
      ...
      // Server returns UNSORTED; the helper must sort priority-first.
...
describe('P8b unpublishPublicly orchestration', () => {
  it('re-registers the unpublished revision + re-announces under the category rid + flips local status', async () => {
    ...
    // Re-registered the UNPUBLISHED revision (rev 2) WITH the stored snapshot pieces.
    ...
    // Re-announced under the category rid with the unpublished record (raw ann verb).
    ...
    // Local row flipped to unpublished revision 2.
    ...
    // Re-unpublishing is guarded (already unpublished).
  ...
  it('honest guards: unknown publication and non-owner are refused without side effects', async () => {
    ...
    // The publication stays active after refused attempts.
  ...
  it('no local snapshot -> directory-only removal, reported honestly', async () => {
    ...
    // Drop the stored snapshot pieces to simulate a publication published before P8b.
    ...
    // No host re-register was attempted (no pieces to send).
