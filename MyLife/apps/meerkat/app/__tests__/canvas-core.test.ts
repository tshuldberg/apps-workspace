/**
 * Plan 56 C1: the Canvas app core + node registry. Proves:
...
} from '../(root)/data/canvas-node-registry-core';
import {
  addCanvasMark,
  addCanvasStroke,
  applyRenderPrefs,
...
describe('C2: profile canvases + copy-forward', () => {
  it('ensureMyProfileCanvas creates once (kind profile, subject self) and reuses', async () => {
    const { ensureMyProfileCanvas } = await import('../(root)/data/canvas-core');
    const db = freshDb();
  ...
  it('listMyProfileDesigns lists only my verified profile canvases with node counts', async () => {
    const { ensureMyProfileCanvas, listMyProfileDesigns } = await import('../(root)/data/canvas-core');
    const db = freshDb();
  ...
  it('copy-forward copies my nodes as NEW signed events, skips unresealable assets, refuses foreign designs', async () => {
    const { ensureMyProfileCanvas, copyProfileDesignForward } = await import('../(root)/data/canvas-core');
    const db = freshDb();
...
describe('C2: sticker layer over threads (feature 12)', () => {
  it('sticks signed stickers over messages, grouped by anchor, honoring receiver dials', async () => {
    const { addThreadSticker, ensureThreadOverlay, listThreadStickers, setCanvasAuthorMuted: mute } = await import('../(root)/data/canvas-core');
    void mute;
    const db = freshDb();
  ...
  it('a sticker is exactly one of glyph or pack image; removal is author-or-curator tombstone', async () => {
    const { addThreadSticker, listThreadStickers, removeThreadSticker } = await import('../(root)/data/canvas-core');
    const db = freshDb();
...
describe('C2: canvas posts (4.5)', () => {
  it('body token round-trips and rejects junk; the canvas is author-signed kind post', async () => {
    const { canvasPostBody, createCanvasPostCanvas, parseCanvasPostBody } = await import('../(root)/data/canvas-core');
    const db = freshDb();
...
describe('C2: page templates (feature 52)', () => {
  it('every host preset applies fully through the registry (zero skips)', async () => {
    const { CANVAS_PAGE_TEMPLATES, applyCanvasTemplateNodes, createPageCanvas } = await import('../(root)/data/canvas-core');
    const db = freshDb();
  ...
  it('export -> import round-trips through the codec as the importer\'s own signed nodes', async () => {
    const { CANVAS_PAGE_TEMPLATES, applyCanvasTemplateNodes, createPageCanvas, exportCanvasTemplate, importCanvasTemplate } = await import('../(root)/data/canvas-core');
    const db = freshDb();
...
describe('C2: per-channel theme overrides + bubble skins (features 3-4)', () => {
  it('the topper policy carries closed themeExtras; channelThemeExtras reads them; forgeries never land', async () => {
    const { channelThemeExtras, ensureCanvas: ensure } = await import('../(root)/data/canvas-core');
    const db = freshDb();
...
describe('C3: the Plaza pixel board (features 6/18)', () => {
  it('owner creates the Plaza; members place; the board and timelapse derive from verified rows', async () => {
    const { ensurePlaza, placePlazaPixel, resolvePlaza } = await import('../(root)/data/canvas-core');
    const db = freshDb();
