...
import { acceptFriendInvite, getFriendInvite } from '@mylife/db';
...
export async function POST(
  ...
  const db = getAdapter();
  const accepted = acceptFriendInvite(db, inviteId, actorUserId);
  if (!accepted) {
    return NextResponse.json({ error: 'Invite cannot be accepted.' }, { status: 400 });
  }
