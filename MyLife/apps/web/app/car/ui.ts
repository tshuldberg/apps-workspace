...
export const CAR_ACCENT = 'var(--accent-car)';
...
export function formatMiles(value: number | null | undefined) {
  return `${(value ?? 0).toLocaleString()} mi`;
}
...
export const shellStyles: Record<string, CSSProperties> = {
  ...
  hero: {
    ...
    background:
      'linear-gradient(145deg, rgba(59,130,246,0.18), rgba(18,18,26,0.94) 58%)',
    display: 'grid',
    gap: '1rem',
  },
  heroTop: {
  ...
  navLinkActive: {
    ...
    padding: '0.5rem 0.9rem',
    borderRadius: '999px',
    border: '1px solid var(--accent-car)',
