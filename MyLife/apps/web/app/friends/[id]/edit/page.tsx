...
import { fetchPerson, editPerson, removePerson } from '../../actions';
import type { PersonRecord, PersonUpdate, RelationshipType, EnergyTag } from '@mylife/friends';
...
export default function EditPersonPage() {
  ...
  useEffect(() => {
    ...
    void fetchPerson(params.id).then((p) => {
      ...
      setHowMet(p.how_met ?? '');
      setWhereMet(p.where_met ?? '');
      setWhenMet(p.when_met ?? '');
      setBirthday(p.birthday ?? '');
      setCity(p.city ?? '');
      setInterestsText(p.interests?.join(', ') ?? '');
      if (p.energy_tag) {
        setEnergyTag(p.energy_tag as EnergyTag);
        setShowMore(true);
      }
      if (p.frequency_goal_days) {
        setFrequencyDays(String(p.frequency_goal_days));
        setShowMore(true);
      }
      setLoading(false);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await editPerson(person.id, updates);
  ...
  }, [person, name, relationshipType, howMet, whereMet, whenMet, birthday, city, interestsText, energyTag, frequencyDays, router]);

  const handleDelete = useCallback(async () => {
    if (!person) return;
    if (!confirm('Delete this person? This will permanently remove them and all associated data.')) return;
    try {
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <Link href={`/friends/${person.id}`} style={{ color: TEXT_SEC, textDecoration: 'none', fontSize: 14 }}>
          &larr; Back
        </Link>
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: TEXT }}>Edit</h1>
        <div style={{ width: 60 }} />
      </div>
