...
function assertSizesAscending(sizes: number[]): void {
  ...
  for (let index = 1; index < sizes.length; index += 1) {
    if (sizes[index] <= sizes[index - 1]) {
      throw new Error('Sizes must be strictly increasing.');
    }
  }
}
...
export async function assertComplexitySlope<TInput>(
  ...
  const {
    ...
    attempts = 3,
    expected = 'linear',
    maxRatios = DEFAULT_RATIO_BY_EXPECTED[expected],
  } = options;

  assertSizesAscending(sizes);

  const budgetAt = (index: number): number =>
    maxRatios[index - 1] ?? maxRatios.at(-1) ?? 2.8;
...
export async function runDeterministicFuzz<TCase>(
  ...
  for (let index = 0; index < options.iterations; index += 1) {
    await options.assertCase(options.makeCase(rng, index), index);
  }
}
