-- BestChef public-launch pending moderation reports queue.
-- Mirrors the local rc_pending_reports SQLite table. Used as a best-effort
-- cloud backup for moderation reports that failed to insert into bc_flags
-- on first attempt (offline, transient outage). The local row is the
-- source of truth; the sweeper retries until cloud insert succeeds.
