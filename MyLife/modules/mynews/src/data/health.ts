/**
 * MyNews service-health classification (plan 48 WP11).
 *
 * The database returns raw numbers (queue depths, ages in seconds, worker
 * heartbeats, threshold rows). Turning those into a status is a judgement, so it
 * lives here in TypeScript next to its tests rather than inside the SQL, and the
 * moderator console and the mynews-health edge function classify identically.
 *
 * Edge twin: supabase/functions/_shared/mynews-health.ts. The edge runtime is
 * Deno and cannot import from this workspace package, so the twin is a mirror
 * pinned by a drift test that runs BOTH implementations over the same fixtures
 * and asserts identical output. Change one, change both.
 *
 * Fail-closed rules, all of them deliberate:
 *   - A component with no threshold row is 'unknown', never 'ok'. An operator
 *     who deletes a threshold row must not thereby silence the alarm.
 *   - A worker that has never recorded a run is 'unknown' with a null age, never
 *     a fabricated healthy heartbeat.
 *   - A worker whose last run reported failure is at least 'warn' regardless of
 *     how recent it was: "it ran five minutes ago" is not health if it failed.
 *   - Unreachable database means 'down'. It never degrades to 'ok' because no
 *     rows came back.
 */
