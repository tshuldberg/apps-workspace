...
export function createShelf(
  ...
  db.execute(
    `INSERT INTO bk_shelves (id, name, slug, icon, color, is_system, sort_order, book_count, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
export function getShelf(db: DatabaseAdapter, id: string): Shelf | null {
  const rows = db.query<Shelf>(
    `SELECT * FROM bk_shelves WHERE id = ?`,
    [id],
  );
...
export function getShelves(db: DatabaseAdapter): Shelf[] {
  return db.query<Shelf>(
    `SELECT * FROM bk_shelves ORDER BY sort_order, name`,
  );
}

export function getSystemShelves(db: DatabaseAdapter): Shelf[] {
  return db.query<Shelf>(
    `SELECT * FROM bk_shelves WHERE is_system = 1 ORDER BY sort_order`,
  );
}
...
export function updateShelf(
  ...
  db.execute(
    `UPDATE bk_shelves SET ${fields.join(', ')} WHERE id = ?`,
    values,
  );
}
...
export function deleteShelf(db: DatabaseAdapter, id: string): void {
  ...
  db.execute(`DELETE FROM bk_shelves WHERE id = ? AND is_system = 0`, [id]);
}

/**
 * Recalculate book_count for a shelf from the bk_book_shelves junction table.
 */
export function refreshShelfCount(db: DatabaseAdapter, shelfId: string): void {
  db.execute(
    `UPDATE bk_shelves SET book_count = (
       SELECT COUNT(*) FROM bk_book_shelves WHERE shelf_id = ?
     ) WHERE id = ?`,
    [shelfId, shelfId],
  );
}
...
/**
 * Get book counts grouped by shelf from the junction table.
 */
