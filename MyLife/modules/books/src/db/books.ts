...
const DEFAULT_BOOK_LIMIT = 500;

export function getBooks(db: DatabaseAdapter, filters?: BookFilters): Book[] {
  let sql = 'SELECT b.* FROM bk_books b';
  const params: unknown[] = [];
  const where: string[] = [];

  if (filters?.shelf_id) {
    sql += ' INNER JOIN bk_book_shelves bs ON b.id = bs.book_id';
    where.push('bs.shelf_id = ?');
    params.push(filters.shelf_id);
  }

  if (filters?.tag_id) {
    sql += ' INNER JOIN bk_book_tags bt ON b.id = bt.book_id';
  ...
  const sortCol = SORT_COLUMNS[filters?.sort_by ?? 'created_at'] ?? SORT_COLUMNS.created_at;
  const sortDir = filters?.sort_dir === 'ASC' ? 'ASC' : 'DESC';
  sql += ` ORDER BY ${sortCol} ${sortDir}`;

  const limit = filters?.limit ?? DEFAULT_BOOK_LIMIT;
  sql += ' LIMIT ?';
  params.push(limit);
  if (filters?.offset) {
    sql += ' OFFSET ?';
...
const BOOK_UPDATE_COLUMNS = new Set([
  'title', 'subtitle', 'authors', 'isbn_10', 'isbn_13',
  'open_library_id', 'open_library_edition_id', 'cover_url', 'cover_cached_path',
  'publisher', 'publish_year', 'page_count', 'subjects', 'description',
  'language', 'format', 'added_source',
]);
...
export function updateBook(
  ...
  for (const [key, value] of Object.entries(updates)) {
    if (!BOOK_UPDATE_COLUMNS.has(key)) continue;
    fields.push(`${key} = ?`);
    values.push(value);
  }
  ...
  db.execute(
    `UPDATE bk_books SET ${fields.join(', ')} WHERE id = ?`,
    values,
  );
}

export function deleteBook(db: DatabaseAdapter, id: string): void {
  db.execute(`DELETE FROM bk_books WHERE id = ?`, [id]);
}
...
export function getBookByISBNLocal(db: DatabaseAdapter, isbn: string): Book | null {
  const rows = db.query<Book>(
    `SELECT * FROM bk_books WHERE isbn_13 = ? OR isbn_10 = ?`,
    [isbn, isbn],
  );
  return rows.length > 0 ? rows[0] : null;
}
...
export function getBookCount(db: DatabaseAdapter): number {
  const rows = db.query<{ count: number }>(`SELECT COUNT(*) as count FROM bk_books`);
