...
export function createSession(
  ...
  db.execute(
    `INSERT INTO bk_reading_sessions (id, book_id, started_at, finished_at, current_page, status, dnf_reason, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
export function getSession(db: DatabaseAdapter, id: string): ReadingSession | null {
  const rows = db.query<ReadingSession>(
    `SELECT * FROM bk_reading_sessions WHERE id = ?`,
    [id],
  );
...
export function getSessions(
  ...
  let sql = 'SELECT * FROM bk_reading_sessions';
  ...
  params.push(filters?.limit ?? DEFAULT_SESSION_LIMIT);
...
export function getSessionsForBook(db: DatabaseAdapter, bookId: string): ReadingSession[] {
  return db.query<ReadingSession>(
    `SELECT * FROM bk_reading_sessions WHERE book_id = ? ORDER BY created_at DESC`,
    [bookId],
  );
}
...
export function updateSession(
  ...
  db.execute(
    `UPDATE bk_reading_sessions SET ${fields.join(', ')} WHERE id = ?`,
    values,
  );
}
...
export function getCurrentlyReading(db: DatabaseAdapter): ReadingSession[] {
  return db.query<ReadingSession>(
    `SELECT * FROM bk_reading_sessions WHERE status = 'reading' ORDER BY started_at DESC`,
  );
}
