...
export function createGoal(
  ...
  db.execute(
    `INSERT INTO bk_reading_goals (id, year, target_books, target_pages, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
...
export function getGoal(db: DatabaseAdapter, id: string): ReadingGoal | null {
  const rows = db.query<ReadingGoal>(
    `SELECT * FROM bk_reading_goals WHERE id = ?`,
    [id],
  );
...
export function getGoalForYear(db: DatabaseAdapter, year: number): ReadingGoal | null {
  const rows = db.query<ReadingGoal>(
    `SELECT * FROM bk_reading_goals WHERE year = ?`,
    [year],
  );
  return rows.length > 0 ? rows[0] : null;
}
...
export function updateGoal(
  ...
  db.execute(
    `UPDATE bk_reading_goals SET ${fields.join(', ')} WHERE id = ?`,
    values,
  );
}
...
export function getGoalProgress(db: DatabaseAdapter, year: number): GoalProgress | null {
  ...
  const pageRows = db.query<{ total: number | null }>(
    `SELECT COALESCE(SUM(b.page_count), 0) as total
     FROM bk_reading_sessions rs
     INNER JOIN bk_books b ON rs.book_id = b.id
  ...
  return {
    ...
    booksRead,
    pagesRead,
    bookPercent: goal.target_books > 0
      ? Math.round((booksRead / goal.target_books) * 100)
      : 0,
    pagePercent: goal.target_pages && goal.target_pages > 0
