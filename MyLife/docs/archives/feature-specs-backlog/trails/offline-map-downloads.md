# Feature Spec: Offline Map Downloads
...
- **Depends On:** none (foundational infrastructure for all Trails map features)
- **Blocks:** weather overlay, wrong-turn alerts, turn-by-turn navigation, route planning, trail database integration

## Business Context

### Why This Feature Exists
Hikers lose cell signal on every trail. Offline maps are the #1 reason users pay for AllTrails ($27-54/yr) and Gaia GPS ($40/yr). Without offline maps, the Trails module is unusable in its core use case: navigating trails in national parks, state forests, and remote coastal areas. This is the foundational infrastructure that every other Trails feature depends on. Wrong-turn alerts, route planning, and navigation all need a map that works without network. AllTrails reports offline maps as their highest premium conversion driver among 20M+ users.
...
| Gaia GPS | Yes | Yes ($39.99/yr) | Download entire topo/satellite layers by drawn bounding box. Backcountry focus. Multiple map source options (USGS topo, satellite, NatGeo). |
| Komoot | Yes | Yes ($29.99 one-time per region) | Download by named region. Includes turn-by-turn voice nav offline. 35M+ users. Charges per region, not subscription. |
| Strava | Partial | Yes ($79.99/yr) | Offline route sync but limited map tile caching. Focused on segments/performance, not navigation. |

### Target User
AllTrails free-tier users (20M+) who hit the "download map" paywall every time they hike. Day hikers and weekend backpackers who need reliable maps in areas with no cell coverage: national parks (Yosemite, Zion, Grand Canyon), state forests, coastal trails. Users currently paying $27-54/yr to AllTrails primarily for this one feature. Also Gaia GPS users ($40/yr) who want a simpler UX without the backcountry complexity.
...
```
modules/trails/src/
  types.ts                      -- New OfflineRegion, DownloadProgress, RegionCatalogEntry schemas
  db/schema.ts                  -- New tr_offline_regions table (migration V2)
  db/crud.ts                    -- CRUD for offline regions
  definition.ts                 -- Add V2 migration, bump schemaVersion to 2
  index.ts                      -- Export new offline functions and types
  offline/                      -- NEW directory
    ...
    storage.ts                  -- Disk usage calculation, cleanup, free space check
...
- **External:**
  - Mobile: `@rnmapbox/maps` with `OfflineManager` API for tile pack downloads
  - Web: `maplibre-gl` with custom tile source falling back to IndexedDB cache
  - Tile source: OpenMapTiles / Protomaps free vector tiles (no API key, self-hostable, .pmtiles format)
- **Cross-Module:** Surf module's map package (`packages/maps/`) contains Mapbox/MapLibre config. Shared tile sources and base map styles can be reused. Score: 2 (mild cross-module benefit).
...
- **Multiple regions with overlapping tiles:** Tiles stored by z/x/y path so identical tiles are naturally deduplicated. Region size_bytes tracks per-region usage for display but actual disk use may be lower due to overlap.
...
- **Extremely slow network:** Show estimated time remaining. Allow user to cancel. Do not time out prematurely (some tile servers are slow for large regions).
...
- [ ] **AC-12:** Downloaded region list and available region list are visually separated
...
- [ ] **TC-8:** estimateRegionTileCount returns accurate counts for given bounding box and zoom range
...
- [ ] **NC-5:** Must NOT allow downloading regions larger than 50,000 tiles without explicit user confirmation
...
20. Start a download, tap "Cancel" mid-download
21. Verify: download stops, no partial region in downloaded list -- corresponds to AC-10
22. Pan map to an area with no downloaded region
23. Verify: grey placeholder tiles shown, no crash -- corresponds to AC-11
24. Verify: "Downloaded" and "Available" sections are visually distinct -- corresponds to AC-12
...
- [ ] `/browse` -- navigate to the offline maps screen, verify all 6 states render
...
- 4 tables in V1 migration (tr_trails, tr_recordings, tr_waypoints, tr_photos)
- Geo engine with haversine distance, elevation gain, pace, duration, calorie estimation
...
- `modules/trails/src/db/schema.ts` -- V2 migration SQL for `tr_offline_regions` table and indexes
...
- `modules/trails/src/index.ts` -- Export new offline functions and types
...
- **Web offline is a stretch goal:** IndexedDB tile caching with Service Worker is architecturally different from mobile's file-system approach. Mobile is the primary target.
- **No tile server self-hosting in V1:** Uses Protomaps/OpenMapTiles CDN. Self-hosting for cost control is a future optimization.
...
- Use `expo-file-system` `documentDirectory` (persists across updates) not `cacheDirectory` (may be purged by OS).
- Wrong-turn alerts (the other A-tier Trails feature) depends on this offline maps infrastructure being in place.
